...
\emph{provided the prompts are carefully designed}.  Models with a
128k-token context window and moderate temperatures can
...
pattern-conformant information such as the document category, the machine number,
and the article number is extracted reliably with regular expressions, reaching
an F1 around 0.94 to 1.0 on the manually checked Belastungsanzeige sample.  Where
a machine number was not stated in the source document, it was recorded as such
rather than guessed.  Narrative content such as the problem, its cause, and the
semantic tags does not follow a fixed pattern and therefore requires LLM
reasoning, which is where rule-based methods fall short.
