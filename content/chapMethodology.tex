...
%
Before settling on PyMuPDF4LLM, five alternative libraries were evaluated
on a test set of 15~PDFs (5~Gutschrift, 5~Lagerbelastung,
5~Belastungsanzeige).  A tool scores one point per PDF where all expected
fields are correctly extracted.  Table~\ref{tab:tool-comparison} summarises
the results.

\begin{table}[htbp]
    \centering
    \caption{PDF extraction tool comparison (score: number of PDFs with
             all fields correctly extracted out of~15).}
...
PyMuPDF4LLM was the only tool to achieve a perfect score across all three
document categories.  Additionally, all tools performed comparably in terms
of speed, with the exception of Docling, which was approximately 1.5$\times$
slower.
